...
if [ -n "$IMAGE" ]; then
    echo "Using $IMAGE as the custom image."
    DEFAULT_TF_IMAGE="$IMAGE"
else
    echo "Using the default image --> $DEFAULT_TF_IMAGE"
fi
echo "======================================"
echo "RUNNING THIS COMMAND "
echo "docker run -v $PWD:$PWD -w $WORKINGDIR/ --env-file terraform_env_variables.env $DEFAULT_TF_IMAGE $ARGS"
echo "======================================"
#exporting all the env variables 
echo "$ADDITIONALPARAMS"  > terraform_env_variables.env 
echo "===========secrets==============="
cat terraform_env_variables.env 
echo "======================================"
#RUNNING Terraform init 
docker run -v $PWD:$PWD -w $PWD/$WORKINGDIR/ $DEFAULT_TF_IMAGE init
docker run -v $PWD:$PWD -w $PWD/$WORKINGDIR/ --env-file terraform_env_variables.env $DEFAULT_TF_IMAGE $ARGS','SHELL','f','now()',1,'now()',1);
...
(nextval('id_seq_plugin_step_variable'),(SELECT ps.id FROM plugin_metadata p inner JOIN plugin_step ps on ps.plugin_id=p.id WHERE p.name='Terraform Cli' and ps."index"=1 and ps.deleted=false),'IMAGE','STRING','The terraform image to use','t','t','docker.io/hashicorp/terraform:latest',null,'INPUT','NEW',null,1,null,null,'f','now()',1,'now()',1),
